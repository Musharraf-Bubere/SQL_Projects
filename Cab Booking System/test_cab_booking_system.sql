CREATE DATABASE test_booking_system;

USE test_booking_system;
...
ORDER BY 
 ...
 -- Insights
-- Top customers have high engagement, showing loyalty and frequent usage.

-- Their ride patterns and locations can be targeted for promotions or premium subscriptions.

-- If their cancellation rate is low, they are stable revenue generators.

-- Retention strategies (e.g. loyalty discounts, priority bookings) can be designed for them.

-- For churn prediction, customers with declining completed bookings over time should be flagged.
...
--  Insights
-- Frequent cancellations (>30%) can indicate:

-- Change in travel plans or scheduling issues

-- Dissatisfaction with cab service (e.g. delays, driver behavior)

-- Price sensitivity if cheaper alternatives exist

-- Testing behavior (booking multiple cabs and choosing one)

-- Action items for business:

-- Analyze cancellation reasons (CancellationReason column) for patterns.

-- Reach out to frequent cancellers for feedback surveys.

-- Implement cancellation fees or stricter policies if cancellations are unjustified.

-- Offer incentives (e.g. “complete your next booking for ₹50 off”) to encourage usage.
...
-- Insights
-- The company’s peak demand day is Friday (hypothetical here).

-- -- Actionable strategies:

-- -- Increase active driver count on peak days to reduce wait time.

-- -- Offer surge pricing or incentives to drivers to remain online during busy hours.

-- -- Run promotional campaigns to spread out bookings to non-peak days.

-- -- Schedule vehicle maintenance on low-demand days (e.g. Tuesday) to maximize availability on peak days.
...
--  Insights & Strategies
-- Reasons for low ratings may include:

-- Poor driving behavior (rash, unsafe)

-- Unprofessional attitude (rude, late arrivals)

-- Vehicle cleanliness issues

-- Strategies to improve performance:

-- Conduct one-on-one performance reviews with these drivers.

-- Provide targeted training in customer service and safety standards.

-- Introduce a mentorship program pairing low-rated drivers with high performers.

-- Monitor their upcoming trips with real-time feedback alerts.

-- Implement incentive-based performance improvement plans to motivate behavior change.
...
-- Insights
-- These drivers are more willing to accept long-distance bookings, possibly indicating:

-- Preference for higher fares per trip

-- Routes aligned with their operating area

-- Comfort with inter-zone trips or highway driving

-- Operational considerations:

-- Long trips may result in fewer total trips per day (affecting utilization %).

-- Drivers covering long distances could be targeted for intercity or premium services.

-- Strategic suggestions:

-- Incentivize long-trip drivers during off-peak hours to balance supply.

-- Analyze if they are consistently working specific corridors for optimized dispatch planning.
...
--  Insights
-- A high driver cancellation % can indicate:

-- Poor reliability: driver often rejects trips after accepting them.

-- Operational problems: vehicle issues, scheduling conflicts.

-- Intentional cancellations: driver cancels short fares for better-paying rides.

-- Route preference: refusing trips to certain areas.
...
-- Identify months with peak revenue (e.g. June ₹67,800)

-- Investigate dip months (e.g. July ₹45,000) to determine causes:
...
-- Increase marketing campaigns in low revenue months.

-- Optimize driver incentives during peak demand periods to maximize fulfilled trips.

-- Use trends for financial forecasting and resource planning.
...
LIMIT 3;
 ...
 -- Insights
-- These top 3 routes have highest demand, indicating:

-- Regular commuters (office routes)

-- Popular city connectivity corridors

-- Strategic recommendations:

-- Allocate more cabs to these routes during peak hours to reduce waiting times.

-- Consider dynamic pricing (surge) during heavy demand windows.

-- Deploy premium cab options if routes cater to higher-income zones.

-- Analyze trip timings to further optimize driver dispatch schedules.
...
-- Insights
-- ✅ Possible observed trends:
-- High-rated drivers (e.g. 4.8 – 4.9) tend to have more trips and higher earnings.

-- Low-rated drivers may:

-- Have fewer trips assigned (due to customer ratings/complaints).

-- Prefer short trips or frequently cancel, reducing earnings.

-- 🔗 Correlation analysis:
-- While the above table suggests a positive correlation between ratings and earnings, statistical correlation (Pearson or Spearman) analysis is required for confirmation in Python or advanced SQL.
...
-- Insights
-- High waiting time locations (e.g. Malad, Borivali):
-- Possible Causes:
-- Low driver density in these areas
-- Longer distances between cabs and pickup points
-- Traffic congestion delaying driver arrival
-- Business Impact:
-- Increased customer dissatisfaction → potential churn
-- Reduced trip completions if customers cancel due to delays
-- Low waiting time areas (e.g. Andheri, Kurla):
-- Well-covered by drivers
-- Higher trip turnover → good operational efficiency
...
-- Driver delayed: Operational inefficiency or poor driver punctuality.

-- Changed plan: Customer-driven; unavoidable to some extent.

-- Alternate transport: Indicates competitor switching or dissatisfaction with service.

-- No show: Driver arrives but customer cancels, possibly due to long waiting time.
...
ORDER BY
 ...
 -- Insights
-- Short trips generate the highest total revenue, despite lower per-trip fares, due to high volume.

-- Long trips have higher average fares but lower frequency, contributing less to overall revenue.
...
--  Insights
-- Sedans generate higher total revenue due to higher trip volume, despite lower per-trip fares.

-- SUVs have higher average fares per trip, indicating premium pricing or long-distance preference.
...
-- Low total bookings (low frequency)

-- Long duration since last ride (high recency days)
...
-- Insights
-- Weekday bookings are higher in volume, indicating routine commuting demand.

-- Weekend bookings have higher average fares, suggesting longer leisure or family trips.